	...
	<TabItem label="Interactive Terminal">
		...
		export default {
			async fetch(request: Request, env: Env): Promise<Response> {
				...
				// Create interactive terminal with PTY support
				...
				await pty.write('echo "Hello from sandbox"\n');
...
<Feature header="Build interactive terminals" href="/sandbox/guides/interactive-terminals/" cta="Learn about PTY support">

Create real-time terminal interfaces with PTY support. Build collaborative terminals, interactive shells, and terminal-based applications with xterm.js integration.
